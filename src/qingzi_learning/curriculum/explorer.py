...
document.querySelectorAll(".explorer-node").forEach(element=>{element.addEventListener("click",()=>showNode(element.dataset.nodeId));element.addEventListener("dblclick",()=>setCollapsed(element,!collapsed.has(element.dataset.nodeId)));element.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();showNode(element.dataset.nodeId)}else if(event.key==="ArrowLeft"){event.preventDefault();setCollapsed(element,true)}else if(event.key==="ArrowRight"){event.preventDefault();setCollapsed(element,false)}})});applyFilters();
'''
