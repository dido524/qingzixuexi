...
<div class="explorer-workspace">
  ...
  <div id="explorer-canvas-wrap">
    <svg id="mastery-graph" viewBox="0 0 1500 {height}" xmlns="http://www.w3.org/2000/svg" aria-label="数学掌握知识图谱">
...
def _node(node: dict[str, Any], point: tuple[float, float]) -> str:
    ...
    return (
        ...
        f'data-source-types="{source_types}" data-source-ids="{source_ids}" role="button" tabindex="0">'
...
def _audit_panel(model: dict[str, Any]) -> str:
    ...
    return f'<section id="mapping-audit"><h2>待整理映射</h2><p>这些标签不会影响正式数学掌握图。</p><ul>{rows}</ul></section>'
...
#explorer-canvas-wrap{flex:1;min-height:560px;overflow:auto;background:linear-gradient(90deg,rgba(226,233,246,.32) 1px,transparent 1px),linear-gradient(rgba(226,233,246,.32) 1px,transparent 1px);background-size:28px 28px}#mastery-graph{display:block;width:100%;min-width:980px;height:100%;min-height:560px;touch-action:none}.explorer-edge{fill:none;stroke:#cad4e3;stroke-width:2}.explorer-edge.relation{stroke:#7d8da8;stroke-dasharray:7 5;marker-end:url(#explorer-arrow);opacity:.7}.explorer-edge.kind-applies_to{stroke:#8c79c6}.explorer-edge.kind-prerequisite{stroke:#527fb6}.explorer-edge.kind-deepens_to{stroke:#469778}.explorer-edge.kind-confusable{stroke:#d38a47}
...
@media print{#filter-rail,#mapping-audit{display:none!important}#explorer-canvas-wrap{min-height:150mm;background:none;overflow:hidden}#mastery-graph{min-width:0;min-height:150mm}#weak-appendix{page-break-before:always;margin:0;border:0}}
'''
...
function applyFilters(){const stage=document.getElementById("filter-stage").value,grade=document.getElementById("filter-grade").value,source=document.getElementById("filter-source").value,mastery=document.getElementById("filter-mastery").value,competency=document.getElementById("filter-competency").value,sourceType=document.querySelector('input[name="source-type"]:checked').value;const keep=new Set();Object.values(data.nodes).forEach(node=>{const sourceIds=node.sources.map(item=>item.source_id),sourceTypes=node.sources.map(item=>item.source_type);let match=(!stage||node.stages.includes(stage))&&(!grade||node.grades.includes(grade))&&(!source||sourceIds.includes(source))&&(!mastery||node.mastery.state===mastery)&&(!competency||node.competencies.includes(competency))&&(!sourceType||sourceTypes.includes(sourceType));if(shortcoming)match=match&&["weak","unstable"].includes(node.mastery.state);if(match){keep.add(node.id);ancestors(node.id,keep);if(shortcoming)data.edges.filter(edge=>edge.kind==="prerequisite"&&edge.target===node.id).forEach(edge=>{keep.add(edge.source);ancestors(edge.source,keep)})}});document.querySelectorAll(".explorer-node").forEach(element=>{const visible=keep.has(element.dataset.nodeId)&&!descendantOfCollapsed(element.dataset.nodeId);element.hidden=!visible;element.classList.toggle("is-dimmed",shortcoming&&!["weak","unstable"].includes(element.dataset.state)&&!data.rootIds.includes(element.dataset.nodeId))});document.querySelectorAll(".explorer-edge").forEach(edge=>{const a=document.querySelector(`[data-node-id="${edge.dataset.source}"]`),b=document.querySelector(`[data-node-id="${edge.dataset.target}"]`);edge.hidden=!a||!b||a.hidden||b.hidden});document.getElementById("graph-empty").style.display=keep.size?"none":"grid";announce(`当前显示 ${[...keep].length} 个知识点`)}
document.querySelectorAll("#filter-rail select,#filter-rail input").forEach(control=>control.addEventListener("change",applyFilters));document.getElementById("toggle-filters").onclick=()=>rail.classList.toggle("is-collapsed");document.getElementById("shortcoming").onclick=event=>{shortcoming=!shortcoming;event.currentTarget.setAttribute("aria-pressed",String(shortcoming));document.body.classList.toggle("shortcoming-active",shortcoming);applyFilters()};document.getElementById("expand-all").onclick=()=>{collapsed.clear();applyFilters()};document.getElementById("print-explorer").onclick=()=>window.print();document.getElementById("fit-explorer").onclick=()=>{graph.setAttribute("viewBox",graph.getAttribute("viewBox").replace(/^.*? .*? /,"0 0 "));graph.scrollIntoView({block:"nearest"})};
function escapeText(value){return String(value).replace(/[&<>"']/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[char])}
function showNode(id){const node=data.nodes[id];document.querySelectorAll(".explorer-node").forEach(item=>item.classList.toggle("is-selected",item.dataset.nodeId===id));const incoming=data.edges.filter(edge=>edge.target===id),outgoing=data.edges.filter(edge=>edge.source===id);const prerequisites=incoming.filter(edge=>edge.kind==="prerequisite").map(edge=>data.nodes[edge.source].label);const downstream=outgoing.map(edge=>data.nodes[edge.target].label);const sources=node.sources.map(item=>`${item.source_label}（${item.status==="confirmed"?"已确认":"待确认"}）`).join("、")||"暂无课程映射";const rate=node.mastery.weighted_rate==null?"暂无":`${Math.round(node.mastery.weighted_rate*100)}%`;details.innerHTML=`<h2>${escapeText(node.label)}</h2><p class="muted">${escapeText(node.stages.join("、"))} · ${escapeText(node.grades.join("、")||"拓展")}</p><h3>前置知识路径</h3><p>${escapeText(prerequisites.join("、")||"暂无已核实前置关系")}</p><h3>后续与应用</h3><p>${escapeText(downstream.join("、")||"暂无已核实下游关系")}</p><h3>教材与训练来源</h3><p>${escapeText(sources)}</p><h3>掌握证据</h3><p>加权正确率 ${rate} · 确认证据 ${node.mastery.evidence_count} 题 · 最近记录 ${escapeText(node.mastery.last_seen_at||"暂无")}</p><h3>确认题目</h3><p>${node.mastery.evidence_count?`共 ${node.mastery.evidence_count} 条确认作答证据；可从知识库题目记录查看原题。`:"暂无已确认题目。"}</p>`;announce(`已选择${node.label}`)}
document.querySelectorAll(".explorer-node").forEach(element=>{element.addEventListener("click",()=>showNode(element.dataset.nodeId));element.addEventListener("dblclick",()=>{collapsed.has(element.dataset.nodeId)?collapsed.delete(element.dataset.nodeId):collapsed.add(element.dataset.nodeId);applyFilters()});element.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();showNode(element.dataset.nodeId)}})});applyFilters();
'''
