...
def _prompt(document: CapturedDocument) -> str:
    ...
    return (
        ...
        "question_id 必须原样保留卷面上印刷的完整题号（包括大题、小题和序号层级，例如2（1）①），"
        "不得输出q1、p1_q1等内部编号；没有印刷题号时才用‘第1页-第1题’这类可定位编号。"
        "所有 question_id 在整份资料内必须唯一，page 对应输入页码。"
        "每题 reason 使用2到4句简明中文：先说明判断依据，再说明关键规则或解题步骤；"
        "错误或部分正确时要指出具体错误并说明怎样改正，正确时也要说明为什么正确；"
        "若 needs_review，要明确缺少什么信息以及家长应核对什么。不得只写‘正确’、‘错误’或‘答案不对’。"
