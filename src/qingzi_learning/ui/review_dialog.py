...
class ReviewDialog:
    def __init__(self, parent, dialog_id, on_save, on_close, open_path,
        ...
        self.item, self.items = None, ()
        self.window = tk.Toplevel(parent, bg=BG)
        self.window.title("待家长确认")
        width = max(680, min(1060, self.window.winfo_screenwidth() - 80))
        height = max(480, min(820, self.window.winfo_screenheight() - 110))
        ...
        self.batch_button = tk.Button(actions, text="确认本批全部正确题", command=self.confirm_correct_batch,
        ...
        tk.Label(self.action_bar, text="批量操作依据单轮 AI 判断", bg=CARD, fg=MUTED,
        ...
        self._card("AI 判断依据", (("原判断", "status"), ("判断理由", "reason"),
                          ("原置信度", "confidence")))
        choice_card = tk.Frame(self.content, bg=CARD, padx=14, pady=10,
                               highlightbackground="#eedee8", highlightthickness=1)
        choice_card.pack(fill="x", pady=(0, 10))
        tk.Label(choice_card, text="家长最终判断", bg=CARD, fg=PINK,
                 font=("Microsoft YaHei UI", 11, "bold")).pack(anchor="w")
        self.status_var = tk.StringVar(master=self.window)
        choices = tk.Frame(choice_card, bg=CARD)
        ...
        for label, variable in (("修正后的参考答案（可选）", self.answer_var), ("复核备注（可选）", self.note_var)):
            tk.Label(choice_card, text=label, bg=CARD, fg=MUTED).pack(anchor="w", pady=(5, 3))
            tk.Entry(choice_card, textvariable=variable).pack(fill="x")
    ...
    def show(self, items, message=""):
        ...
        self.progress_var.set(f"本次剩余 {len(items)} 道待确认题目")
        ...
        batch_count = len(self._batch_items())
        self.batch_button.configure(state="normal" if batch_count else "disabled",
                                    text=f"确认本批全部正确题（{batch_count}）")
    ...
    def _render_preview(self):
        ...
        try:
            ...
            with Image.open(preview_path) as source_image:
                ...
                preview.thumbnail((min(900, max(300, self._preview_width - 65)), 400))
    ...
    def confirm_correct_batch(self):
        selected = self._batch_items()
        if not selected or self.on_batch is None:
            return
        if not messagebox.askyesno("确认正确题", f"本次有 {len(selected)} 道题被单轮 AI 判为正确。\n"
                                  "确认后它们会一起计入知识库；错误、部分正确和不确定的题仍逐题确认。\n\n"
                                  "要确认这批正确题吗？", parent=self.window):
            return
    ...
    def destroy(self):
        ...
        self.detail_vars = {}
        for name in ("meta_var", "progress_var", "status_var", "answer_var", "note_var", "message_var", "_photo"):
